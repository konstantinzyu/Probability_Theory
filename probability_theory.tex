...
\begin{document}
	...
	Прямой образ меры $ \xi_*\prob $ (напомним, что он определяется правилом $ \xi_*\prob(B) = \prob(\xi^{-1}(B)) $) 
	на борелевской $ \sigma $-алгебре $ \calB $ подмножеств $ \RR $
	называется \defin{распределением случайно величины $ \xi $}{distribution}.
