...
\begin{document}
	...
	или \defin{вырожденным распределением}{singular-distribution}. Используя табличку можем записать
	$$ \xi \sim 
	\begin{tabular}{c}
		$ C $ \\
		$ 1 $ 
	...
	Рассмотрим случайную величину $ \xi $ принимающую каждое значение из набора $ x_1, x_2, \ldots, x_n $
	с вероятностью $ \tfrac{1}{n} $. Распределение случайной величины $ \xi $ называется 
	\defin{дискретным распределением}{discrete-uniform}. В виде таблицы:
	$$ \xi \sim 
	\begin{tabular}{ccc}
		$ x_1 $ & $ \ldots $ & $ x_n $ \\
	...
	равной $ p^kq^{n - k} $, где $ k $ --- это число единиц в кортеже $ (\varepsilon_1, \ldots, \varepsilon_n) $.
	...
	что $ \xi $ принимает значения от $ 0 $ до $ n $, причём значение $ k $ принимается ей с вероятностью
	$ C_{n}^{k}p^kq^{n - k} $.
	Распределение такой случайной величины $ \xi $ называется 
	\defin{биномиальным распределением}{binomial}.
	В виде таблицы:
	$$ \xi \sim 
